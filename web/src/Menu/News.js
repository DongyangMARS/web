      ...
      <main className="main-content">
        <div className="noticebox">
          ...
          <div className="flex justify-end mt-3 gap-2">
            <button className="px-3 py-1 bg-green-600 text-white rounded edit-btn " onClick={handleAdd}>
              뉴스 추가
            </button>
            <button className="px-3 py-1 bg-gray-600 text-white rounded reset-btn" onClick={handleResetAll}>
              전체 초기화
            </button>
          </div>
                ...
                safeNews.map((item) => (
                  <React.Fragment key={item.id}>
                    <tr>
                      ...
                      <td className="p-2">
                        <button
                          className="text-blue-600 underline edit-btn"
                          onClick={() => (openRow === item.id ? setOpenRow(null) : handleEdit(item))}
                        >
                          {openRow === item.id ? "닫기" : "수정"}
                        </button>
                        <button className="px-3 py-1 bg-gray-500 text-white rounded ml-2 edit-btn" onClick={() => handleReset(item.id)}>
                          초기화
                        </button>
                        <button className="px-3 py-1 bg-red-600 text-white rounded ml-2 delete-btn" onClick={() => handleDelete(item.id)}>
                          삭제
                        </button>
                      </td>
                    </tr>
                    ...
                    {openRow === item.id && (
                      <tr>
                        <td colSpan={3} className="p-4 bg-gray-50" style={{gap : "10px"}}>
                          <div className="mb-2 center-items">
                            <label className="block mb-1 text-sm font-medium ">제목</label>
                            <input
                              type="text"
                              value={editTitle}
                              onChange={(e) => setEditTitle(e.target.value)}
                              className="w-full border p-2 rounded title-textarea"
                          ...
                          <div className="mb-2 center-items">
                            ...
                            <textarea
                              ...
                              className="w-full h-24 border p-2 rounded main-textarea"
                            />
                          </div>

                          <div className="flex justify-end gap-2 mt-2">
                            <button className="px-3 py-1 bg-blue-500 text-white rounded edit-btn1" onClick={() => handleSave(item.id)}>
                              저장
                            </button>
                          </div>
                        </td>
