                            ...
                            {faqs.map((faq) => (
                                <React.Fragment key={faq.id}>
                                    <tr>
                                        ...
                                        <td className="p-2">
                                            <button
                                                className="text-blue-600 underline reset-btn"
                                                onClick={() =>
                                            ...
                                            <button className="px-3 py-1 bg-gray-500 text-white rounded reset-btn" onClick={() => handleReset(faq.id)}>초기화</button>
                                            <button className="px-3 py-1 bg-red-600 text-white rounded delete-btn" onClick={() => deleteFaq(faq.id)}>삭제</button>
                                        </td>
                                    </tr>
                                    {openRow === faq.id && (
                                        <tr>
                                            <td colSpan={2} className="p-2 bg-gray-50 ">
                                                <input type="text" value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="w-full border p-2 rounded title-textarea" placeholder="질문(제목)" style={{ marginBottom: "8px" }}/>
                                                <textarea
                                                    value={editContent}
                                                    onChange={(e) => setEditContent(e.target.value)}
                                                    className="w-full border p-2 rounded main-textarea "
                                                    rows={6} cols={80}
                                                />
                                                <div className="flex justify-end mt-2 gap- ">
                                                    <button className="px-3 py-1 bg-blue-500 text-white rounded edit-btn" onClick={() => handleSave(faq.id)}>저장</button>
